...
suspendCmd="rpower hostname suspend"
queryCmd="rpower hostname stat"

#query before take action, if the host power status is already suspend, print ok
...
#check the result, if the host power status is suspend, print ok.
echo "$resultStrHost" | sed "s/\://" | awk '{if($2 == "suspend") {print $1" 0"}}'

#shrink the resultStr
resultStrNode=`echo "$resultStrNode" | grep -v "\<suspend\>[ ]*$"`

#shrink the req hostname and send the rpower command
namelist=`echo "$resultStrNode" | sed "s/\:.*$/,/" | xargs | sed "s/,$//" | sed "s/, /,/g"`
if [ "$namelist" != "" ]; then
    command=`echo "$suspendCmd" | sed "s/hostname/$namelist/"`
    resultStrNode=`$command 2>&1 < /dev/null`
    updateResultHost
    echo "$resultStrHost" | sed "s/\://" | awk ' {if($2 == "suspend"  ) {print $1" 0"} else {print $1" 1"} }'
fi
